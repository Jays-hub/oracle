...
5. **Coexist** — a second restaurant can onboard without seeing or corrupting the first
   (app-DB tenancy from W5; physical seam partitioning when the second *real* tenant arrives, W9).
...
| **W7** | Production hosting + security hardening | TLS, real deploy target, per-request CSRF tokens, managed persistent secrets, rate limiting on the funnels, backups (app DB + `data/raw/`), monitoring/structured logs, the price-history retention policy, and the **recorded Postgres decision** (execute the swap iff hosted concurrency demands it). Closes the W0/W2 pre-deploy bundle. | **Durable** | the first remote user / pilot deploy |
