...
| 9 | Name-based joins across the seam (`normalize_name()` defends them; CONTRACT forward note, issue #4) | Stable `item_id` carried across the seam | **W6, deferred beyond W9** — W9 delivered the physical partition only; item_id was reviewed and consciously deferred (`W9_review.md` MINOR-1, accepted 2026-07-16), not built |
...
- The `food_cost` schema definition (W6). The `item_id` format was reviewed at W9 and consciously
  deferred, not confirmed — it remains open, deferred to whichever later on-ramp phase takes it up
  (`data/CONTRACT.md`, `W9_review.md` MINOR-1, accepted 2026-07-16).
